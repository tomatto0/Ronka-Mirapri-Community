...
import { usePosts } from "@/app/(pages)/hooks/usePosts";
import { useSessionStorage } from "@/app/(pages)/hooks/useSessionStorage";
import Itemrank from "@/app//components/Itemrank";
import { filter_tag_init_state, job_category, job_category_group } from "@/app/utils/constants";

import EditButton from "@/app/components/EditButton";
import ErrorContainer from "@/app/components/ErrorContainer";
import SearchParamsHandler from "@/app/(pages)/util/SearchParamsHandler";
import SkeletonMain from "@/app/components/SkeletonMain";
...
export default function Page_home() {
  ...
  const post_chunk = useMemo(() => {
    if (!posts.data) return [[]];
    const post_list = posts.data.pages.reduce<PostInform[]>((acc, _) => {
      return [...acc, ...(_.data ?? [])];
    }, []);
    return post_list.reduce<PostInform[][]>((acc, _, i) => {
      if (i % 4 === 0) {
  ...
  function update_filter() {
    ...
    if (typeof window !== "undefined") {
      window.sessionStorage.setItem("filter", JSON.stringify({ filter, filter_tag }));
    }
  }
  ...
  function job_delete(job: string) {
    function job_groupize(job: string[]) {
      ...
      job.sort((a, b) => (order_map.get(a) ?? Infinity) - (order_map.get(b) ?? Infinity));
      return job;
    }
    ...
    <Suspense>
      <main className="fill">
        ...
        {posts.status === "pending" ? (
          ...
          <p>
            Error:{" "}
            {posts.error instanceof Error ? posts.error.message : "An unknown error occurred"}
          </p>
        ) : post_chunk.length === 0 ? (
            ...
            <div className="post-container">
              {post_chunk.length > 0 && (
                <div className="post-container-row" key={0}>
                  {post_chunk[0].map((post: PostInform, index: number) => (
                    <PostThumbnail post={post} key={`post-${post.index}`} priority={index === 0} />
                  ))}
                </div>
              )}
            </div>
